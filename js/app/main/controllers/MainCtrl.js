 ...
 */
function MainCtrl($scope, $timeout, $location, $filter, $routeParams, Data, Config) {
    ...
    function setCurrentLanguage(){
        $scope.currentLanguage = $filter('filter')($scope.languages, {language_id:$scope.langId})[0];
    }   
    ...
    function setCurrentTable() {
        $scope.currentTable = $filter('filter')($scope.tables, {table_id:$scope.tableId})[0];
    ...
    $scope.reloadData = function(){   
        $timeout(function() {
            $scope.$broadcast('reloadData', {lang: $scope.langId , table:$scope.tableId});
            getTableParams($scope.tableId);
        });       
    }
}